...
- **댓글 분석 (영상별 자세히)**: `commentThreads`로 최상위 댓글 **최대 500개**(5페이지=영상당 최대 5유닛,
  ...
  - **분류 정규식은 UI 언어와 무관하게 한/영 고정**(`AD_PATTERNS`, `COMMENT_BUCKETS[].pats[].r`).
    분석 대상 채널·댓글이 대부분 한국어이므로 번역하지 않는다. 번역되는 것은 "표시 라벨"뿐(§7 i18n).
...
- **다국어(i18n)**: topnav 우측 드롭다운(국기+언어명 3옵션: 한국어 / 중국어 간체 / 영어). 모든 표시 문자열은
  `I18N` 사전(ko/zh/en, 키 101개)에서 관리하고 `t(key, params)`로 치환(`{n}` 자리표시자). 누락 키는 ko 폴백.
  - 정적 마크업은 `data-i18n`/`data-i18n-ph` 속성, 동적 렌더는 `t()` 호출. 숫자 포맷도 언어별 로케일(ko-KR/zh-CN/en-US).
  - **언어 전환 시 이미 렌더된 결과(변환/분석/펼쳐진 댓글 패널)도 즉시 재렌더** — 데이터는 `LAST_CONVERT`/`LAST_WINDOWS`/
    `COMMENT_CACHE`에 유지되므로 **API 재호출 없음(쿼터 0)**. 펼쳤던 샘플 `<details>`·커스텀 키워드 입력값도 보존.
  - 선택은 `localStorage.yt_lang`에 저장, 최초엔 `navigator.language`(zh*/en* 감지, 그 외 ko).
  - 윈도우 라벨은 `labelKey`/`labelParams`로 저장해 렌더 시점에 번역(문자열로 저장하면 언어 전환 못 따라옴).
  - 버킷/근거 라벨은 `nameKey`/`lKey`(번역 키), 커스텀 키워드는 원문 그대로.
  - 서버발 문자열(`server.mjs` 로그·에러)은 한국어 고정 — 미번역 허용(로컬 개인 도구 전제).
...
- **i18n**: 인라인 스크립트 추출 후 `node --check` + 사전 일관성 스크립트(3언어 키 세트 일치, `data-i18n`/`t()`/
  `nameKey`/`lKey` 참조 키가 사전에 존재, `{x}` 자리표시자 언어 간 일치). 목업 데모를 언어별(zh/en/ko)로 생성해
  headless Chrome 스크린샷으로 전 화면 번역·재렌더 확인.
- **CJK 오염 스캔 범위**: `index.html`은 이제 중국어 사전을 정당하게 포함하므로 **스캔 제외**.
  대신 `AGENTS.md·plan.md·design.md·README.md·server.mjs·cli.mjs`만 검사(한/일문자 혼입 방지).
  문서에 언어명을 쓸 때도 한자 표기(예: 중국어 간체를 한자로 적은 형태) 대신 "중국어 간체"처럼 한글로 적어 스캔 유효성을 유지한다.
...
- (보류) 중국어 댓글 분류 키워드(`COMMENT_BUCKETS` zh 패턴) — 분석 대상 댓글이 대부분 한국어라 효용 낮음.
  필요해지면 `pats`에 언어별 배열을 추가하는 방식 검토(현재는 라벨만 번역).
- (보류) 서버발 문자열(`server.mjs`) 번역 — 로컬 개인 도구 전제라 한국어 고정 허용.
